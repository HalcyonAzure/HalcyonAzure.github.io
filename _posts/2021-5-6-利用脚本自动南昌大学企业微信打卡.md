...
2. 一个能用SMTP登入并且发信的邮箱（也可以使用类似Server酱的消息推送平台，如果不知道邮箱的SMTP信息如何获得可以百度）
...
2. 在电脑的浏览器打开了链接以后，按F12，在右上角找到`Network`，并打开。
...
4. 创建一个`ClockIn.py`的脚本文件，并且按注释编辑以下脚本
   ...
   # 设置邮件服务器信息
   mail_host = 'smtp.xxx.net'  # SMTP服务器
   mail_user = '123456@yeah.net'  # SMTP登入用户名
   mail_pass = '********'  # SMTP登入密码，国内通常是单独的授权码，而不是登入密码。
   sender = '123456@yeah.net'  # 发信人，通常就是自己SMTP的邮件地址
   receivers = ['youremail@qq.com', '123456@yeah.net']  # 修改youremail@qq.com为自己的QQ邮箱即可
   
   # 设置email的内容
   message = MIMEText(ans, 'plain', 'utf-8')
   message['Subject'] = '打卡通知'
   message['From'] = "ClockIn<"+sender+">"
   message['To'] = "ClockIn<"+receivers[0]+">"
   # 登入并且发送邮件
   try:
   ...
   >receivers之所以要再添加一次'123456@yeah.net'，是因为在使用网易给QQ邮箱发邮件（Outlook不存在这个问题）的时候会报SPM垃圾邮件拒收的问题
   >只需要在receivers里面再添加一次自己的邮箱就可以正常发送了。由于只使用了网易的服务，不明是个例还是通常情况。

# 运行脚本并测试

1. 在`Ubuntu`下安装了`python3`以后，输入指令。

   ```shell
   python3 /path/to/ClockIn.py
   ```
